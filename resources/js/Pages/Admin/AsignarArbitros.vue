...
const props = defineProps({
    ...
    isEspectador: { type: Boolean, default: false }
});
const getAsignado = (partido, funcion) => {
    if (!partido || !partido.designaciones) return 'Sin asignar';
    const desig = partido.designaciones.find(d => d.funcion === funcion);
    return desig && desig.user ? `${desig.user.apellido}, ${desig.user.name}` : 'Sin asignar';
};
let intervalEspectador = null;
const form = useForm({
...
onMounted(() => {
    ...
    if (props.isEspectador) {

        intervalEspectador = setInterval(() => {
            router.reload({ only: ['partidos'], preserveScroll: true, preserveState: true });
        }, 5000);
        return;
    }
...
const disciplinaSeleccionada = ref('TODAS');
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div v-if="import_summary && mostrarResumen"
            class="mb-8 bg-green-50 border border-green-200  p-5 shadow-sm relative overflow-hidden animate-fade-in">
            <div class="absolute top-0 right-0 w-24 h-24 bg-green-500/10 rounded-full blur-2xl"></div>
            <div class="flex items-start gap-4 relative z-10">
                <div
                    class="w-10 h-10 bg-green-500 rounded-full flex items-center justify-center text-white shrink-0 shadow-sm mt-0.5">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                    </svg>
                </div>
                <div class="flex-1">
                    <h3 class="text-green-800 font-black text-[15px] uppercase tracking-widest mb-1">Carga Completada
                        con Éxito</h3>
                    <p class="text-green-700 text-sm font-medium mb-4">El sistema procesó el archivo excel y extraído
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                        <div class="bg-white rounded-lg p-3 border border-green-100 shadow-sm flex items-center gap-3">
                            <div class="w-8 h-8 rounded-full bg-green-50 flex items-center justify-center"><span
                                    class="text-green-600 font-black">{{ import_summary.total_filas }}</span></div>
                            <div>
                                <p class="text-[10px] text-gray-500 font-black uppercase tracking-wider">Filas Leídas
                                </p>
                                <p class="text-xs font-bold text-[#0D1B3E]">Total en Excel</p>
                            </div>
                        </div>
                        <div class="bg-white rounded-lg p-3 border border-green-100 shadow-sm flex items-center gap-3">
                            <div class="w-8 h-8 rounded-full bg-blue-50 flex items-center justify-center"><span
                                    class="text-blue-600 font-black">{{ import_summary.nuevos }}</span></div>
                            <div>
                        ...
                        <div class="bg-white rounded-lg p-3 border border-green-100 shadow-sm flex items-center gap-3">
                            <div class="w-8 h-8 rounded-full bg-amber-50 flex items-center justify-center"><span
                                    class="text-amber-600 font-black">{{ import_summary.actualizados }}</span></div>
                            <div>
                                <p class="text-[10px] text-amber-500 font-black uppercase tracking-wider">Omitidos /
                                    Act.</p>
                                <p class="text-xs font-bold text-[#0D1B3E]">Repetidos</p>
                            </div>
                        </div>
                    </div>
                </div>
                <button @click="mostrarResumen = false"
                    class="text-green-500 hover:text-green-700 bg-green-100 hover:bg-green-200 rounded-full p-1.5 transition-colors focus:outline-none">
        ...
        <div class="mb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
            ...
            <div v-if="partidos.length > 0"
                ...
                <span class="text-[#0D1B3E] font-black text-[12px] uppercase tracking-widest">{{ partidos.length }}
                    Partidos pendientes</span>
            </div>
        </div>
        ...
        <div v-if="partidos.length === 0"
            ...
            <p class="text-[14px] text-[#6B7280] font-medium">No hay partidos pendientes de asignación en este momento.
                Subí una nueva planilla excel para comenzar.</p>
            <Link :href="route('admin.importar.index')"
                ...
                Ir a Importar Excel</Link>
        </div>

        <form v-else @submit.prevent="submit" class="flex flex-col gap-6">

            <div
                class="bg-white p-5  border border-[#E5E7EB] shadow-sm grid grid-cols-1 md:grid-cols-3 gap-6 z-10 relative">

                <div class="flex flex-col gap-2">
                    <label class="block text-[11px] font-black text-[#6B7280] uppercase tracking-[0.15em] pl-1">1.
                        Seleccionar Cancha</label>
                    <div class="relative">
                        <select v-model="canchaSeleccionada"
                            ...
                            <option v-for="cancha in canchasDisponibles" :key="cancha" :value="cancha">{{ cancha }} ({{
                                partidosPorCancha[cancha].length }} partidos)</option>
                        </select>
                        <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none"><svg
                                class="w-4 h-4 text-[#D4A843]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex flex-col gap-2">
                    <label class="block text-[11px] font-black text-[#6B7280] uppercase tracking-[0.15em] pl-1">2.
                        Filtrar por Día</label>
                    <div class="relative">
                        <select v-model="fechaSeleccionada"
                            class="block w-full text-[13px] font-bold border-[#E5E7EB] focus:border-[#D4A843] focus:ring-[2px] focus:ring-[#D4A843]/20 rounded-lg py-2.5 pl-10 pr-4 bg-gray-50 hover:bg-white transition-colors cursor-pointer text-[#0D1B3E] shadow-sm appearance-none">
                            <option value="TODAS">Mostrar toda la semana</option>
                            <option v-for="fecha in fechasDeCancha" :key="fecha" :value="fecha">Solo ver {{
                                formatearFechaCompleta(fecha) }}</option>
                        </select>
                        <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none"><svg
                                class="w-4 h-4 text-[#D4A843]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="flex flex-col gap-2">
                    <label class="block text-[11px] font-black text-[#6B7280] uppercase tracking-[0.15em] pl-1">3.
                        Filtrar por Tipo</label>
                    <div class="relative">
                        <select v-model="disciplinaSeleccionada"
                            class="block w-full text-[13px] font-bold border-[#E5E7EB] focus:border-[#D4A843] focus:ring-[2px] focus:ring-[#D4A843]/20 rounded-lg py-2.5 pl-10 pr-4 bg-gray-50 hover:bg-white transition-colors cursor-pointer text-[#0D1B3E] shadow-sm appearance-none">
                            <option value="TODAS">Todas las disciplinas</option>
                            <option v-for="disciplina in disciplinasDisponibles" :key="disciplina" :value="disciplina">
                                {{ disciplina }}</option>
                        </select>
                        <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none"><svg
                                class="w-4 h-4 text-[#D4A843]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div v-if="canchaSeleccionada && Object.keys(partidosTabla).length > 0"
                class="bg-white border border-[#E5E7EB]  shadow-sm overflow-hidden flex flex-col">
                ...
                <div class="bg-[#0D1B3E] px-6 py-4 border-b border-[#D4A843]/30 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="text-white font-black uppercase tracking-widest text-[15px]">Cancha: {{
                            canchaSeleccionada }}</h3>
                    </div>
                </div>
                ...
                <div class="overflow-x-auto max-h-[60vh] custom-scrollbar relative">
                    <table class="w-full text-left whitespace-nowrap">
                        <thead class="bg-[#F9FAFB] sticky top-0 z-30 shadow-sm border-b border-[#E5E7EB]">
                            <tr>
                                <th scope="col"
                                    class="px-5 py-3.5 border-r border-[#E5E7EB] text-[10px] font-black text-[#6B7280] uppercase tracking-[0.15em]">
                                    Hora</th>
                                <th scope="col"
                                    class="px-5 py-3.5 border-r border-[#E5E7EB] text-[10px] font-black text-[#6B7280] uppercase tracking-[0.15em] text-center">
                                    Cat.</th>
                                <th scope="col"
                                    class="px-5 py-3.5 text-[10px] font-black text-[#6B7280] uppercase tracking-[0.15em] text-right">
                                    Local</th>
                                <th scope="col"
                                    class="px-2 py-3.5 text-[10px] font-black text-[#6B7280] uppercase tracking-[0.15em]">
                                </th>
                                <th scope="col"
                                    class="px-5 py-3.5 border-r border-[#E5E7EB] text-[10px] font-black text-[#6B7280] uppercase tracking-[0.15em]">
                                    Visitante</th>
                                <th scope="col"
                                    class="px-4 py-3.5 border-r border-[#E5E7EB] text-[10px] font-black text-[#0D1B3E] uppercase tracking-[0.15em] bg-blue-50 min-w-[180px]">
                                    Principal</th>
                                <th scope="col"
                                    class="px-4 py-3.5 border-r border-[#E5E7EB] text-[10px] font-black text-[#0D1B3E] uppercase tracking-[0.15em] bg-blue-50 min-w-[180px]">
                                    Asistente 1</th>
                                <th scope="col"
                                    ...
                                    Asistente 2</th>
                                <th scope="col"
                                    class="px-4 py-3.5 text-[10px] font-black text-[#0D1B3E] uppercase tracking-[0.15em] bg-blue-50 min-w-[180px]">
                                    Asistente 3 / Crono</th>
                            </tr>
                        </thead>
                                ...
                                <tr class="bg-[#F7F5F0] border-b border-[#E5E7EB]">
                                    <td colspan="9" class="px-5 py-2.5">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span
                                                class="text-[#0D1B3E] font-black uppercase tracking-widest text-[13px]">{{
                                                    formatearFechaCompleta(fecha) }}</span>
                                        </div>
                                    </td>
                                </tr>

                                <tr v-for="item in partidosDia" :key="item.datos.id"
                                    ...
                                    <td class="px-5 py-3 border-r border-[#E5E7EB] bg-white group-hover:bg-gray-50">
                                        <span class="text-[#D4A843] font-black text-[13px] block">{{
                                            item.datos.hora_inicio.substring(0, 5) }}</span>
                                        <span class="text-[9px] text-gray-400 font-bold uppercase">{{
                                            item.datos.disciplina }}</span>
                                    </td>
                                    <td class="px-5 py-3 border-r border-[#E5E7EB] text-center"><span
                                            class="px-2 py-1 bg-gray-100 text-gray-600 rounded text-[10px] font-black uppercase tracking-wider border border-gray-200">{{
                                                item.datos.categoria }}</span></td>
                                    <td class="px-5 py-3 text-right font-black text-[13px] text-[#0D1B3E] uppercase">{{
                                        item.datos.equipo_local }}</td>
                                    <td class="px-2 py-3 text-center text-[10px] font-bold text-gray-400">VS</td>
                                    <td
                                        class="px-5 py-3 font-black text-[13px] text-[#0D1B3E] uppercase border-r border-[#E5E7EB]">
                                        {{ item.datos.equipo_visitante }}</td>

                                    <td
                                        class="px-2 py-2 border-r border-[#E5E7EB] bg-blue-50/10 group-hover:bg-blue-50/30 transition-colors align-top">


                                        <div v-if="isEspectador"
                                            class="w-full text-[11px] font-black uppercase tracking-wider bg-white border border-[#E5E7EB] rounded py-2 px-2 shadow-sm text-center">
                                            <span
                                                :class="getAsignado(item.datos, 'ARBITRO PRINCIPAL') === 'Sin asignar' ? 'text-red-400 animate-pulse' : 'text-[#0D1B3E]'">
                                                {{ getAsignado(item.datos, 'ARBITRO PRINCIPAL') }}
                                            </span>
                                        </div>
                                        ...
                                        <template v-else>
                                            ...
                                            <select v-model="form.asignaciones[item.indiceOriginal].principal_id"
                                                ...
                                                <option value="" disabled class="text-gray-400">-- Seleccionar --
                                                </option>
                                            ...
                                            <p v-if="form.asignaciones[item.indiceOriginal].principal_id && chequearHistorial(form.asignaciones[item.indiceOriginal].principal_id, item.datos.equipo_local, item.datos.equipo_visitante)"
                                                ...
                                                <svg class="w-3 h-3 shrink-0" fill="none" stroke="currentColor"
                                                    ...
                                                    chequearHistorial(form.asignaciones[item.indiceOriginal].principal_id,
                                                        item.datos.equipo_local, item.datos.equipo_visitante) }}
                                            </p>
                                        </template>
                                    ...
                                    <td
                                        ...
                                        <div v-if="isEspectador"
                                            class="w-full text-[11px] font-black uppercase tracking-wider bg-white border border-[#E5E7EB] rounded py-2 px-2 shadow-sm text-center">
                                            <span
                                                :class="getAsignado(item.datos, 'ASISTENTE 1') === 'Sin asignar' ? 'text-red-400 animate-pulse' : 'text-[#0D1B3E]'">
                                                {{ getAsignado(item.datos, 'ASISTENTE 1') }}
                                            </span>
                                        </div>
                                        ...
                                        <template v-else>
                                            ...
                                            <p v-if="form.asignaciones[item.indiceOriginal].asistente_id && chequearHistorial(form.asignaciones[item.indiceOriginal].asistente_id, item.datos.equipo_local, item.datos.equipo_visitante)"
                                                ...
                                                <svg class="w-3 h-3 shrink-0" fill="none" stroke="currentColor"
                                                    ...
                                                    chequearHistorial(form.asignaciones[item.indiceOriginal].asistente_id,
                                                        item.datos.equipo_local, item.datos.equipo_visitante) }}
                                            </p>
                                        </template>
                                    </td>
                                    <td
                                        ...
                                        <div v-if="isEspectador"
                                            class="w-full text-[11px] font-black uppercase tracking-wider bg-white border border-[#E5E7EB] rounded py-2 px-2 shadow-sm text-center">
                                            <span
                                                :class="getAsignado(item.datos, 'ASISTENTE 2') === 'Sin asignar' ? 'text-red-400 animate-pulse' : 'text-[#0D1B3E]'">
                                                {{ getAsignado(item.datos, 'ASISTENTE 2') }}
                                            </span>
                                        </div>
                                        ...
                                        <template v-else>
                                            ...
                                            <p v-if="form.asignaciones[item.indiceOriginal].asistente_2_id && chequearHistorial(form.asignaciones[item.indiceOriginal].asistente_2_id, item.datos.equipo_local, item.datos.equipo_visitante)"
                                                ...
                                                <svg class="w-3 h-3 shrink-0" fill="none" stroke="currentColor"
                                                    ...
                                                    chequearHistorial(form.asignaciones[item.indiceOriginal].asistente_2_id,
                                                        item.datos.equipo_local, item.datos.equipo_visitante) }}
                                            </p>
                                        </template>
                                    </td>
                                    <td
                                        class="px-2 py-2 bg-blue-50/10 group-hover:bg-blue-50/30 transition-colors align-top">


                                        <div v-if="isEspectador"
                                            class="w-full text-[11px] font-black uppercase tracking-wider bg-white border border-[#E5E7EB] rounded py-2 px-2 shadow-sm text-center">
                                            <span
                                                :class="getAsignado(item.datos, 'ASISTENTE 3 / CRONO') === 'Sin asignar' ? 'text-red-400 animate-pulse' : 'text-[#0D1B3E]'">
                                                {{ getAsignado(item.datos, 'ASISTENTE 3 / CRONO') }}
                                            </span>
                                        </div>
                                        ...
                                        <template v-else>
                                            ...
                                            <p v-if="form.asignaciones[item.indiceOriginal].asistente_3_id && chequearHistorial(form.asignaciones[item.indiceOriginal].asistente_3_id, item.datos.equipo_local, item.datos.equipo_visitante)"
                                                ...
                                                <svg class="w-3 h-3 shrink-0" fill="none" stroke="currentColor"
                                                    ...
                                                    chequearHistorial(form.asignaciones[item.indiceOriginal].asistente_3_id,
                                                        item.datos.equipo_local, item.datos.equipo_visitante) }}
                                            </p>
                                        </template>
                                    </td>
            ...
            <div
                ...
                <div v-if="!isEspectador"
                    ...
                    <button type="submit" :disabled="form.processing"
                        ...
                        <span v-if="form.processing" class="relative z-10 flex items-center justify-center gap-2"><svg
                                class="animate-spin" width="16" height="16" viewBox="0 0 16 16" fill="none">
                                <circle cx="8" cy="8" r="6" stroke="currentColor" stroke-width="2" stroke-dasharray="20"
                                    stroke-dashoffset="5" />
                            </svg>Procesando...</span>
                        <span v-else class="relative z-10 flex items-center justify-center gap-2"><svg width="15"
                                height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"
                                stroke-linecap="round" stroke-linejoin="round">
                                <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path>
                                <polyline points="22 4 12 14.01 9 11.01"></polyline>
                            </svg>Confirmar Designaciones</span>
                    </button>
                </div>
