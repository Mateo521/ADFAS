...
<template>
    <GuestLayout>
        ...
        <div v-if="status" class="flex items-center gap-2.5 mb-6 px-4 py-3 bg-[#F0FDF4] border border-[#BBF7D0] rounded-lg text-[13px] font-semibold text-[#166534]">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" class="shrink-0">
                <circle cx="8" cy="8" r="7.25" stroke="#166534" stroke-width="1.5"/>
                <path d="M5 8l2 2 4-4" stroke="#166534" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
            {{ status }}
        </div>
        ...
        <form @submit.prevent="submit" class="flex flex-col gap-[22px]" novalidate>
            ...
            <label class="flex items-center gap-2.5 text-[13px] font-semibold text-[#374151] cursor-pointer">
                <Checkbox name="remember" v-model:checked="form.remember" class="w-4 h-4 rounded border-[#9CA3AF] text-[#1E3370] focus:ring-[#1E3370]" />
                <span>Recordarme en este dispositivo</span>
            </label>

            <div class="flex flex-col gap-4 pt-2 border-t border-[#E5E7EB] mt-1">
                <Link v-if="canResetPassword" :href="route('password.request')" class="text-[12px] font-bold text-[#1E3370] hover:text-[#D4A843] hover:underline tracking-[0.3px] transition-colors self-start">
